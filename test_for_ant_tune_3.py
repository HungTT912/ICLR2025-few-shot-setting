...
def main():
    ...
    wandb.init(project='test-BBDM-fewshot-setting',
    ...
    for seed in seed_list:
        ...
        model_load_path = f'results/few_shot/tune_4/{nconfig.task.name}/sampling_lr{nconfig.GP.sampling_from_GP_lr}/initial_lengthscale{nconfig.GP.initial_lengthscale}/delta{nconfig.GP.delta_lengthscale}/initial_outputscale{nconfig.GP.initial_outputscale}/delta{nconfig.GP.delta_variance}/seed{seed}/BrownianBridge/checkpoint/top_model_epoch_100.pth'
        optim_sche_load_path = f'results/few_shot/tune_4/{nconfig.task.name}/sampling_lr{nconfig.GP.sampling_from_GP_lr}/initial_lengthscale{nconfig.GP.initial_lengthscale}/delta{nconfig.GP.delta_lengthscale}/initial_outputscale{nconfig.GP.initial_outputscale}/delta{nconfig.GP.delta_variance}/seed{seed}/BrownianBridge/checkpoint/top_optim_sche_epoch_100.pth'
    ...
    if not os.path.isfile(file_path):
        with open(file_path, 'a') as file:
            header = ['lengthscale','delta_lengthscale', 'variance','delta_variance','eta','alpha','classifier_free_guidance_weight', 'mean (100th)', 'std (100th)', 'mean (80th)', 'std (80th)', 'mean (50th)', 'std (50th)']
    ...
    delta_lengthscale = nconfig.GP.delta_lengthscale 
    ...
    for eta in [0.2, 0.25, 0.5, 0.0, 0.05, 0.1, 0.15]: 
        for w in [-2.0, -1.5, -1, 0, 1, 2 , 3, 4]:  
            for alpha in [0.8, 0.9, 0.95, 1.0]: 
                ...
                with open(file_path, 'a') as file:
                    new_row = [lengthscale,delta_lengthscale, variance, delta_variance, eta, alpha, w, mean_score_100th, std_score_100th, mean_score_80th, std_score_80th, mean_score_50th, std_score_50th]
                    writer = csv.writer(file)
