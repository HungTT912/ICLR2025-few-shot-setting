...
def main():
    ...
    file_path = f'./few-shot-results/tuning_3_result_{nconfig.task.name}_test_{nconfig.testing.type_sampling}.csv'
    ...
    best_ant_params = pd.read_csv(f'./few-shot-results/tuning_3_result_AntMorphology-Exact-v0_test_{nconfig.testing.type_sampling}_l{nconfig.GP.initial_lengthscale}.csv')
    best_ant_params = best_ant_params[best_ant_params['mean (100th)']>0.94] 
    best_ant_params = best_ant_params[['eta','alpha','classifier_free_guidance_weight']].to_numpy()
    
    for eta, alpha, w in best_ant_params: 
        results_100th = []
        results_80th = [] 
        results_50th = []
